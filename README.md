...
- **Formatting toolbar** (icon box below the main toolbar) plus **Format menu**
  ...
  - Heading 1/2, Bullet List, Numbered List, Quote, Code Block — all wrap or prefix the current selection while keeping the Markdown source intact

- **Rendered preview**
  - Markdown rendered with Python-Markdown (fenced code, tables, TOC, codehilite/Pygments)
  - Syntax-colored code blocks with a continuous background color
...
```powershell
python -m venv venv
venv\Scripts\python -m pip install PyQt6 markdown Pygments
```
